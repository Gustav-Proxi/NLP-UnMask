...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Phase} & \textbf{Entry} & \textbf{Exit} & \textbf{Window} \\
\midrule
Rapport    & Session start      & 4 diagnostic Qs  & 0--120s  \\
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Metric} & \texttt{full} & \texttt{no\_pcr} & \texttt{no\_crag} & \texttt{no\_graph} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Ablation results (30 questions/variant, mastery = 0.20). $^\dagger$Answer Chunk Reach Rate = fraction of queries where the answer chunk enters LLM context. Full system = 0.000 is \emph{correct behavior}: PCR excludes answer chunks at cold-start mastery.}
\label{tab:ablation}
\end{table}
