...
We evaluated on 30 QA triples with a four-variant ablation: Hit Rate 1.000,
...
no factual claims, so RAGAS correctly finds zero entailed propositions and
scores it as unfaithful, even though from a tutoring standpoint it is fine.
Answer Relevancy scores how directly the response answers the question, so a
response that guides rather than answers will always score low here.
We think Socratic Purity (4.93/5, LLM-as-Judge) is the right faithfulness proxy for
this task. These results point to a gap in NLP evaluation: there is no standard
metric for groundedness in Socratic dialogue systems.

%% ─────────────────────────────────────────────────────────────
\section{Discussion}

\paragraph{Why benign testing is not enough.}
The most interesting thing from our ablation is that all variants pass the leak
rate test on benign inputs.
If we only tested on benign inputs, we would conclude PCR adds nothing.
This matches the failure mode Dinucu-Jianu et al.\ \cite{tutorrl2025} found, and our
adversarial battery confirms it.
PCR's advantage only shows up when the system is actually under attack.
