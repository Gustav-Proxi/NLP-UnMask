...
\noindent\textbf{System action (Orchestrator, before graph.invoke):}\\
\texttt{study\_focus = ``brachial plexus''}\\
\texttt{learning\_mode = ``visual''}\\
\texttt{diag\_order = [0, 2, 3, 1]} \quad (brachial-plexus questions first)\\
\texttt{visual\_hint\_threshold = 1} \quad (diagram after first incorrect response)
...
\texttt{consecutive\_incorrect = 1 $\geq$ threshold (1)}

\noindent\textbf{System action:} \texttt{cl.Image(path=public/anatomy/brachial\_plexus.png,
display="inline")} appended to next message.
File: Gray809 (public domain, Wikimedia Commons), 108~KB PNG.
...
At $t \geq 840$s the Orchestrator triggers the \texttt{wrapup} phase.
GPT-4o \texttt{response\_format=SessionSummary} produces:

\begin{small}
\begin{verbatim}
overall_assessment:
  "You showed solid recall of the brachial plexus root levels
   (C5-T1) and correctly identified radial nerve wrist drop.
   Trunk formation and teres minor innervation need more work."

topic_reports:
  - concept: brachial_plexus.trunks
    mastery_score: 0.25
    status: needs_review
    honest_feedback: "You struggled here in turns 1 and 4;
      review how C5-C6 form the upper trunk specifically."
  ...
  - concept: peripheral_nerves.radial
    mastery_score: 0.72
    status: mastered
    honest_feedback: "Wrist drop presentation identified
      correctly with good reasoning."

mistake_highlights:
  - "Confused upper trunk formation (C5+C6) with C5 alone."
  - "Did not recall teres minor as second axillary nerve target."

study_recommendations:
  - "Draw the RTTDC mnemonic (Roots-Trunks-Divisions-Cords)."
  - "Practice Phalen's and Tinel's sign sequences out loud."

closing_reflection:
  "If a patient presents with deltoid weakness and lateral arm
   numbness post shoulder dislocation, which nerve and which
   cord of the brachial plexus would you evaluate first?"
\end{verbatim}
\end{small}
